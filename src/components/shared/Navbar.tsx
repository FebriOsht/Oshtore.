...
import { ExternalLink, Menu, X, ChevronRight, ChevronDown, Tag, HelpCircle } from 'lucide-react';
    ...
    <div className="fixed top-4 inset-x-0 z-[100] flex justify-center px-4 pointer-events-none">
      ...
      >
        ...
        <div className="hidden lg:flex items-center gap-1 bg-white/5 p-1 rounded-full border border-white/5">
          ...
          {/* Tautan in-page — hanya muncul di halaman relevan */}
          ...
          {/* Harga & FAQ — sembunyikan saat scrolled agar navbar tidak penuh */}
          {!scrolled && (
            <>
              <a href="/pricing" className={`flex items-center gap-1.5 px-5 py-2 text-sm font-bold rounded-full transition-all ${isPricingPage ? 'bg-indigo-500/20 text-indigo-300' : 'text-slate-300 hover:text-white hover:bg-white/10'}`}>
                <Tag size={13} /> Harga
              </a>
              <a href="/faq" className={`flex items-center gap-1.5 px-5 py-2 text-sm font-bold rounded-full transition-all ${isFaqPage ? 'bg-indigo-500/20 text-indigo-300' : 'text-slate-300 hover:text-white hover:bg-white/10'}`}>
                <HelpCircle size={13} /> FAQ
              </a>
            </>
          )}

          {/* Dropdown Informasi — saat scrolled, Harga/FAQ/Portfolio masuk ke sini */}
          <div
            className="relative"
            onMouseEnter={() => setInfoDropdownOpen(true)}
            onMouseLeave={() => setInfoDropdownOpen(false)}
          >
            <button className={`px-5 py-2 text-sm font-bold rounded-full transition-all flex items-center gap-1 ${isInfoPage || isPricingPage || isFaqPage ? 'bg-white/10 text-white' : 'text-slate-300 hover:text-white hover:bg-white/10'}`}>
              {scrolled ? 'Menu' : 'Informasi'} <ChevronDown size={14} className={`transition-transform duration-300 ${infoDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            <div className={`absolute top-full left-1/2 -translate-x-1/2 pt-4 transition-all duration-300 ${infoDropdownOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible -translate-y-2'}`}>
              <div className="bg-[#0f172a]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-2 w-52 shadow-2xl flex flex-col gap-1">
                {/* Harga & FAQ masuk ke dropdown saat scrolled */}
                {scrolled && (
                  <>
          ...
          {/* Portfolio — sembunyikan saat scrolled, sudah masuk dropdown */}
          {!scrolled && (
